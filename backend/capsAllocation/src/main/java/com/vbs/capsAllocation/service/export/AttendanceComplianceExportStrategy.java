...
/**
 * Export strategy for Attendance Compliance Issues
 */
@Component
public class AttendanceComplianceExportStrategy implements ExportStrategy {

    @Autowired
    private ReportsService reportsService;

    private static final String EXPORT_TYPE = "ATTENDANCE_COMPLIANCE";
    ...
    public byte[] generateExcel(LocalDate startDate, LocalDate endDate, String userName, Map<String, Object> filters) {
        LoggerUtil.logDebug("Generating ATTENDANCE_COMPLIANCE export for user: {}, date range: {} to {}",
        ...
        try (Workbook workbook = new XSSFWorkbook();
            ...
            List<DefaulterDetailDTO> defaulters = reportsService.getAllDefaulters(
                    "Attendance", startDate, endDate, userName, filters);
            ...
            LoggerUtil.logError("Error generating ATTENDANCE_COMPLIANCE export: {}", e.getMessage(), e);
    ...
    /**
     * Create detailed instances sheet showing each attendance issue
     */
    private void createDetailedInstancesSheet(Workbook workbook, List<DefaulterDetailDTO> defaulters,
        ...
        // Header row
        Row headerRow = sheet.createRow(0);
        String[] headers = { "Employee Name", "Email", "Date", "Entry Time", "Exit Time",
                "Late Login Reason", "Late/Early Logout Reason", "Outside Office", "Status" };
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            cell.setCellStyle(headerStyle);
        ...
        for (DefaulterDetailDTO defaulter : defaulters) {
            ...
            try {
                List<IssueDetailDTO> issues = reportsService.getUserIssues(
                        "attendance", ldap, startDate, endDate);
                ...
                for (IssueDetailDTO issue : issues) {
                    ...
                    Cell cell3 = row.createCell(3);
                    cell3.setCellValue(
                            issue.getEntryTimestamp() != null ? issue.getEntryTimestamp().format(DATETIME_FORMATTER)
                                    : "");
                    cell3.setCellStyle(cellStyle);

                    Cell cell4 = row.createCell(4);
                    cell4.setCellValue(
                            issue.getExitTimestamp() != null ? issue.getExitTimestamp().format(DATETIME_FORMATTER)
                                    : "");
                    cell4.setCellStyle(cellStyle);

                    Cell cell5 = row.createCell(5);
                    cell5.setCellValue(issue.getLateLoginReason() != null ? issue.getLateLoginReason() : "");
                    cell5.setCellStyle(cellStyle);

                    Cell cell6 = row.createCell(6);
                    cell6.setCellValue(
                            issue.getLateOrEarlyLogoutReason() != null ? issue.getLateOrEarlyLogoutReason() : "");
                    cell6.setCellStyle(cellStyle);

                    Cell cell7 = row.createCell(7);
                    cell7.setCellValue(issue.getIsOutsideOffice() != null && issue.getIsOutsideOffice() ? "Yes" : "No");
                    cell7.setCellStyle(cellStyle);

                    Cell cell8 = row.createCell(8);
                    cell8.setCellValue(issue.getStatus() != null ? issue.getStatus() : "");
                    cell8.setCellStyle(cellStyle);
                }
            } catch (Exception e) {
                LoggerUtil.logWarn(AttendanceComplianceExportStrategy.class,
