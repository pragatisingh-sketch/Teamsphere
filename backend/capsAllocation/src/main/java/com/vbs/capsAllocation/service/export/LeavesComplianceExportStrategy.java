...
/**
 * Export strategy for Leaves Compliance Issues
 */
@Component
public class LeavesComplianceExportStrategy implements ExportStrategy {

    @Autowired
    private ReportsService reportsService;

    private static final String EXPORT_TYPE = "LEAVES_COMPLIANCE";
    ...
    public byte[] generateExcel(LocalDate startDate, LocalDate endDate, String userName, Map<String, Object> filters) {
        LoggerUtil.logDebug("Generating LEAVES_COMPLIANCE export for user: {}, date range: {} to {}",
        ...
        try (Workbook workbook = new XSSFWorkbook();
            ...
            List<DefaulterDetailDTO> defaulters = reportsService.getAllDefaulters(
                    "Leaves", startDate, endDate, userName, filters);
            ...
            LoggerUtil.logError("Error generating LEAVES_COMPLIANCE export: {}", e.getMessage(), e);
    ...
    /**
     * Create detailed instances sheet showing each leave issue
     */
    private void createDetailedInstancesSheet(Workbook workbook, List<DefaulterDetailDTO> defaulters,
        ...
        // Header row
        Row headerRow = sheet.createRow(0);
        String[] headers = { "Employee Name", "Email", "Start Date", "End Date", "Created At", "Leave Type",
                "Leave Category",
                "Duration", "Application Type", "Status", "Start Time", "End Time" };
        for (int i = 0; i < headers.length; i++) {
            Cell cell = headerRow.createCell(i);
            cell.setCellValue(headers[i]);
            cell.setCellStyle(headerStyle);
        ...
        for (DefaulterDetailDTO defaulter : defaulters) {
            ...
            try {
                ...
                for (IssueDetailDTO issue : issues) {
                    ...
                    Cell cell2 = row.createCell(2);
                    cell2.setCellValue(issue.getFromDate() != null ? issue.getFromDate().format(DATE_FORMATTER) : "");
                    cell2.setCellStyle(dateStyle);

                    Cell cell3 = row.createCell(3);
                    cell3.setCellValue(issue.getToDate() != null ? issue.getToDate().format(DATE_FORMATTER) : "");
                    ...
                    Cell cell5 = row.createCell(5);
                    cell5.setCellValue(issue.getLeaveType() != null ? issue.getLeaveType() : "");
                    cell5.setCellStyle(cellStyle);

                    Cell cell6 = row.createCell(6);
                    cell6.setCellValue(issue.getLeaveCategory() != null ? issue.getLeaveCategory() : "");
                    cell6.setCellStyle(cellStyle);

                    Cell cell7 = row.createCell(7);
                    cell7.setCellValue(issue.getDuration() != null ? issue.getDuration() : "");
                    cell7.setCellStyle(cellStyle);

                    Cell cell8 = row.createCell(8);
                    cell8.setCellValue(issue.getApplicationType() != null ? issue.getApplicationType() : "");
                    ...
                    cell10.setCellValue(issue.getStartTime() != null ? issue.getStartTime().toString() : "");
                    cell10.setCellStyle(cellStyle);

                    Cell cell11 = row.createCell(11);
                    cell11.setCellValue(issue.getEndTime() != null ? issue.getEndTime().toString() : "");
                    cell11.setCellStyle(cellStyle);
                }
            } catch (Exception e) {
                LoggerUtil.logWarn(LeavesComplianceExportStrategy.class,
